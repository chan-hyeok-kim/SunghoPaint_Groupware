...
SELECT
	...
	(
		...
		(
			TIMESTAMPDIFF
			(
				SECOND, ATTENDANCE_START, 
				CASE
					/* 3시간 이상 근무 시 점심 시간 제외 */
					WHEN TIMESTAMPDIFF(SECOND, ATTENDANCE_START, COALESCE(ATTENDANCE_END, ATTENDANCE_START)) >= 3 * 3600 THEN
						CASE
							/* 4시간 이상 근무 시 -1시간 */
							WHEN TIMESTAMPDIFF(SECOND, ATTENDANCE_START, COALESCE(ATTENDANCE_END, ATTENDANCE_START)) >= 4 * 3600 THEN
								COALESCE(ATTENDANCE_END, ATTENDANCE_START) - INTERVAL 1 HOUR
							/*
								3시간 이상 4시간 미만 근무 시
								ex) 3시간 30분 30초 : 30분 30초 제외
							*/
							ELSE
								COALESCE(ATTENDANCE_END, ATTENDANCE_START)
								-
...
FROM
	ATTENDANCE
WHERE
	MONTH(ATTENDANCE_DATE) = MONTH(NOW())
GROUP BY
	EMPLOYEE_ID;